...
from . import summarizer, speechTextAzure, uniqueNames, textSummarizer, todo
import math
import datetime
com_text = []
full_text = []
...
def transcript(request):
    ...
    unique_text_list = uniqueNames.unique_names(com_text[0])
    print(unique_text_list)
    return render(request, 'result.html', {'res': [complete_text.split("."), meeting_start_time, unique_text_list, meeting_title]})
...
def namechange(request):
    ...
    for path in glob.iglob(os.path.join(pwd, '*.mp3')):
        os.remove(path)
    diff_speakers = uniqueNames.unique_names(com_text[0])
    # names = str(request.POST["spnames"])
    # name_list = names.split(" ")[0].split("\r\n")
    for speaker in diff_speakers:
        actual_name = str(request.POST[speaker])
        com_text[0] = com_text[0].replace(speaker, actual_name)
    text = " ".join(full_text)
    summary_text = summarizer.summarize(text)
    # summary_text_ = textSummarizer.textSummarize(text)
    todo_text_ = todo.to_do(text)

    return render(request, "namechange.html", {"text": [com_text[0].split("."), summary_text, todo_text_]})
